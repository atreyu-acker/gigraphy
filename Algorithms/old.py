...
def draw_sections(sections, active_section=0):
    ...
    # calculate shared y range across all sections
    all_y = []
    ...
    fig.subplots_adjust(wspace=0)  # no gap between subplots
    ...
    for i, (section, ax) in enumerate(zip(sections, axes)):
        ...
        # show y axis and labels only for active and completed sections
        if i <= active_section:
        ...
        # hide x ticks on non-active sections to keep clean
        if i != active_section:
            ax.tick_params(axis='x', labelsize=7)
        
        # dots and annotations for active section only
        if i == active_section:
            points = calculate_significant_points(section)
            for px, py in points:
                ax.plot(px, py, 'o', color="black", markersize=5, zorder=5)
                ax.annotate(
                    f"({round(px, 1)}, {round(py, 1)})",
                    xy=(px, py),
                    xytext=(0, 10),
...
def generate_linear_section(y_start):
    y_end = np.random.randint(0, 51)
    m = round((y_end - y_start) / SECTION_WIDTH, 2)
    c = round(y_start, 2)  # always y_start since x starts at 0
    return {
        "y_start": y_start,
        "y_end": y_end,
...
def generate_quadratic_section(y_start):
    vertex_x = np.random.randint(5, SECTION_WIDTH - 5)
    a = np.random.choice([0.05, 0.1, 0.15, -0.05, -0.1, -0.15])
    ...
    # shift so curve starts at y_start
    y_at_zero = c  # since a*0^2 + b*0 + c = c
    y_offset = y_start - y_at_zero
    c = round(c + y_offset, 2)
    vertex_y = round(vertex_y + y_offset, 2)
    ...
    # calculate roots from final equation
    discriminant = b**2 - 4 * a * c
    roots = []
    if discriminant >= 0:
        root1 = round((-b + np.sqrt(discriminant)) / (2 * a), 2)
    ...
    return {
        ...
        "y_end": y_end,
        "section_width": SECTION_WIDTH,
        "x_values": x_values,  # local coordinates 0 to SECTION_WIDTH
        "y_values": y_values,
        "coefficients": {"a": a, "b": b, "c": c},
        "roots": roots,
        "vertex": (round(vertex_x, 2), round(vertex_y, 2)),
        "equation_type": "quadratic"
    }

def generate_level(difficulty):
    equation_types = list(difficulty_map[difficulty])
    np.random.shuffle(equation_types)
    ...
    for eq_type in equation_types:
        if eq_type == "linear":
            section = generate_linear_section(y_current)
        elif eq_type == "quadratic":
            section = generate_quadratic_section(y_current)
        sections.append(section)
        y_current = section["y_end"]
...
def calculate_significant_points(section):
    ...
    if section["equation_type"] == "linear":
        ...
        # y intercept — always at x=0 since local coords
        points.append((0, round(c, 2)))

        # x intercept
        if m != 0:
            x_int = round(-c / m, 2)
            if 0 <= x_int <= section["section_width"]:
                points.append((x_int, 0))
        ...
        # roots
        for root in section["roots"]:
            if 0 <= root <= section["section_width"]:
                points.append((root, 0))

        # vertex
        points.append(section["vertex"])
